...
### Disable Refraction Without Disabling Glass

When a glass component has no meaningful background to refract, disable only
the background lens sampling with `LiquidGlassSettings.refractionEnabled`:
...
GlassCard(
  ...
  child: const Text('Glass material, no background distortion'),
)
```

With `refractionEnabled: false`, background displacement, chromatic dispersion,
and interactive indicator pinch sampling are disabled. Blur, tint, saturation,
lighting, Fresnel, edge absorption, whitening, shape, and interactive geometry
continue to render normally. Existing no-backdrop fallback rules still apply:
Standard uses its synthetic body based on `glassColor`, while Premium resolves
empty backdrop regions through `platformViewFallbackColor`, then `backerColor`,
and finally the historical fallback.

This is also available as a nullable theme override:

```dart
GlassThemeVariant(
  settings: GlassThemeSettings(refractionEnabled: false),
)
```

The default is `true`, so existing surfaces are unchanged. This switch keeps the
