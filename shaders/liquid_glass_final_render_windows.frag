...
//
// Windows 原生兼容变体：uniform 与通用版本保持一致，但编译单元中不包含
// 超椭圆 mode 2 的动态幂函数，规避 Flutter 3.47.x OpenGLESSDF 首屏空白。
...
vec4 analyticRoundedRectSdfAt(vec2 localPoint) {
    // 中文说明：Windows 安全变体只接受 mode 1 圆角矩形；超椭圆由 Dart
    // 宿主回退到既有 geometry texture，避免 OpenGLESSDF 编译 mode 2 失败。
...
void main() {
    ...
    // 中文说明：与主入口共用主体、肩部、顶底核心的连续能量分配；由于本平台
    // headroom 固定为 1.0，视觉仍严格收口于 SDR，但高光几何不会产生分叉。
