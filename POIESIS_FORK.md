...
7. Premium、Standard 与交互指示器共用 `edge_treatment.glsl` 的双层边缘：
   ...
   - 双层细边与物理 `edgeAbsorption` 解耦：先绘制完整围绕轮廓的
     ...
     弯月面变暗，不会移除这两层结构边；
   - 双层结构边内侧新增按几何局部 Y 归一化的面积高光：大尺寸（高度超过 67dp）
     顶部在容器高度 `15%` 处归零（封顶 `10 logical px`），底部覆盖最后 `30%`（封顶 `20 logical px`），
     两端峰值平台分别封顶为 `2 / 3 logical px`（基准 `3%` / `4%`）；当容器高度降至 `24~48dp` 小尺寸
     （如圆球、圆形进度指示器、按钮徽标）时，因曲面顶部水平弦长急剧收缩，通过小尺寸平滑过渡因子自适应
     放宽比例上限（顶部约 `28%`、底部约 `36%`）与峰值平台保底（顶部约 `7%`、底部约 `8%`），确保满曝光平台
     不被 `0.36dp` 结构描边挤占；离开平台后的衰减曲率在小尺寸下平滑软化至 `0.75` 幂次，模拟球面漫射晕染，
     大尺寸小尺寸因子为 0，视觉效果 100% 维持不变。高光仍由背景逐通道加权剩余亮度空间，并额外乘一层
